...
export function inspectGeneration2({ schema, active, archiveNames, state, checksums }) {
  ...
  const creators = new Map(),
    re = /\bCREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"]?([a-z_][a-z\d_]*)[`"]?/gi;
  for (const [migration, raw] of active)
    for (const m of stripSqlComments(raw).matchAll(re)) {
      const table = m[1].toLowerCase(),
        prev = creators.get(table);
      if (prev) findings.push(`BLOCK: ${table} is created twice (${prev}, ${migration}).`);
...
export function inspectRepository(root = ROOT) {
  const schema = join(root, "database", "prisma", "schema.prisma"),
    activeDir = join(root, "database", "prisma", "migrations"),
    archiveDir = join(root, "database", "prisma", "migration-history-archive", "generation-1"),
    statePath = join(root, "database", "prisma", "state", "canonical-state.json"),
    checksumsPath = join(root, "database", "prisma", "state", "migration-checksums.json");
  for (const p of [schema, activeDir, archiveDir, statePath, checksumsPath])
    if (!existsSync(p)) return [`BLOCK: required migration-security source is missing: ${p}`];
  ...
  return inspectGeneration2({
    ...
    active,
    archiveNames: new Set(readMigrationMap(archiveDir).keys()),
    state: JSON.parse(readFileSync(statePath, "utf8")),
    checksums: JSON.parse(readFileSync(checksumsPath, "utf8"))
  });
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const f = inspectRepository();
  if (f.length) {
    for (const x of f) console.error(x);
    console.error(`MIGRATION_SECURITY=BLOCKED (${f.length} findings)`);
    process.exitCode = 1;
  } else {
    console.log("MIGRATION_SECURITY=PASS");
    console.log(
      "Generation 1 is isolated, active migration coverage is complete, frozen checksums match, and required raw-SQL contracts are present."
    );
  }
}
