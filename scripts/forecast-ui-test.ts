...
const manyProducts = Array.from({ length: 15 }, (_, index) =>
  product({
    ...
    sku: index === 14 ? "TARGET-SKU" : `SKU-${index + 1}`,
...
assert.equal(matchesForecastSearch(products[0]!, "SKU-B"), true);
assert.equal(matchesForecastSearch(products[0]!, "480001"), true);
...
assert.equal(
  (forecastPageSource.match(/placeholder="Search product by name or SKU\.\.\."/g) ?? []).length,
  1
);
