...
pdf_path = Path("frontend/src/utils/directPdfExport.ts")
pdf_path.write_text(r'''import { jsPDF } from "jspdf";
...
export function downloadOperationalSummaryPdf(snapshot: InternalReportSnapshot) {
  ...
  drawMetric(doc, margin + (metricWidth + gap), 48, metricWidth, "Recent gross", money(recentGross(snapshot)));
  drawMetric(doc, margin + (metricWidth + gap) * 2, 48, metricWidth, "Receipts", snapshot.completedSales.length.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 3, 48, metricWidth, "Units sold", recentUnits(snapshot).toLocaleString());

  doc.text("Inventory overview", margin, 80);
  drawMetric(doc, margin, 84, metricWidth, "Tracked products", summary.inventory.trackedItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap), 84, metricWidth, "Units on hand", totalUnitsOnHand(snapshot).toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 2, 84, metricWidth, "Low stock", summary.inventory.lowStockItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 3, 84, metricWidth, "Out of stock", summary.inventory.outOfStockItems.toLocaleString());

  const inventoryHealthy = summary.inventory.lowStockItems === 0 && summary.inventory.outOfStockItems === 0;
  const expiryHealthy = summary.expiry.nearExpiryBatches === 0 && summary.expiry.expiredBatches === 0;
  autoTable(doc, {
    body: [
      [
        "Inventory status",
...
export function downloadInventoryReportPdf(snapshot: InternalReportSnapshot) {
  ...
  drawMetric(doc, margin, 27, metricWidth, "Tracked products", summary.inventory.trackedItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap), 27, metricWidth, "Units on hand", totalUnitsOnHand(snapshot).toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 2, 27, metricWidth, "Active products", summary.inventory.availableItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 3, 27, metricWidth, "Inactive / stopped", summary.inventory.unavailableItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 4, 27, metricWidth, "Low stock", summary.inventory.lowStockItems.toLocaleString());
  drawMetric(doc, margin + (metricWidth + gap) * 5, 27, metricWidth, "Out of stock", summary.inventory.outOfStockItems.toLocaleString());
  ...
  autoTable(doc, {
    ...
    head: [["SKU", "Product", "Product status", "Stock status", "On hand", "Available", "Reorder", "Target"]],
...
dialog_path = Path("frontend/src/components/reports/ReportDownloadDialog.tsx")
