...
export function inspectMigrationSources(schema, migrations) {
  ...
  for (const [migration, sql] of sorted) {
    ...
    for (const match of withoutComments.matchAll(createTable)) {
      ...
      if (previous) {
        findings.push(`BLOCK: ${table} is created twice (${previous}, ${migration}). Do not modify applied migration files or replay this chain.`);
      } else {
  ...
  for (const [table, model] of models) {
    if (!tableCreators.has(table.toLowerCase())) {
      findings.push(`BLOCK: Prisma model ${model} maps to ${table}, but no historical migration creates that table.`);
    }
  }
  for (const [table, migration] of tableCreators) {
    if (!models.has(table)) {
      findings.push(`BLOCK: Migration ${migration} creates ${table}, absent from schema.prisma. A schema sync may propose dropping it.`);
    }
  }
  return findings;
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (args.length !== 0 && !(args.length === 2 && args[0] === '--root')) {
    ...
    if (findings.length) {
      ...
      console.error('Read database/prisma/MIGRATION_RECOVERY.md; do not run migrate dev/deploy/reset or db push.');
      process.exitCode = 1;
    } else {
      console.log('MIGRATION_SOURCE_CHECK=PASS; live schema, data, checksums and isolated replay still require verification.');
