...
test("Windows Electron icon is a native multi-resolution ICO used at runtime and packaging", async () => {
  const ico = await readFile(join(electronRoot, "build", "icon.ico"));
  const sizes = readIcoSizes(ico);
  for (const required of ["16x16", "32x32", "48x48", "256x256"]) {
    assert.ok(sizes.includes(required), `Windows icon must include ${required}`);
  }
  assert.equal(sha256(ico), "ce7857516100d6a883ae58794c93eaf4bc76d4fc5ef25d5ffa224fdc2a95ecc4");
