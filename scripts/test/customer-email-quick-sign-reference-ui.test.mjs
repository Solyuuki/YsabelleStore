...
test("Email Quick Sign borrows Recovery proportions without importing Recovery UI", () => {
  ...
  assert.match(
    css,
    /\.customer-email-quick-sign\s*\{[\s\S]*?border:\s*0;[\s\S]*?background:\s*transparent;[\s\S]*?box-shadow:\s*none;/
  );
  assert.match(
    css,
    /\.customer-email-quick-sign__intro h2\s*\{[\s\S]*?font-family:\s*var\(--customer-font-display\)/
  );
  assert.match(css, /\.customer-email-quick-sign__form\s*\{[\s\S]*?width:\s*min\(100%, 31rem\)/);
  assert.match(
    css,
    /\.customer-email-quick-sign__form > \.customer-auth-submit\s*\{[\s\S]*?min-height:\s*56px/
  );
});
...
test("Quick Sign OTP slots remain six full Recovery-proportion boxes", () => {
  ...
  assert.match(
    css,
    /\.customer-verification-code__group\s*\{[\s\S]*?display:\s*grid\s*!important;[\s\S]*?grid-template-columns:\s*repeat\(6, minmax\(0, 1fr\)\)/
  );
  assert.match(
    css,
    /\.customer-verification-code__slot\s*\{[\s\S]*?width:\s*100%;[\s\S]*?min-height:\s*72px/
  );
});
