...
test("customer login is a single centered premium shader card", () => {
  const shaderPath = "frontend/src/styles/customer-auth-login-premium.css";
  assert.equal(existsSync(fileUrl(shaderPath)), true, "premium login shader stylesheet must exist");
  ...
  assert.match(frame, /\{isRegister \? \(/);
  assert.match(frame, /customer-auth-stage__brand/);

  assert.match(css, /\.customer-auth-page--login\s*\{[\s\S]*?display:\s*grid;[\s\S]*?place-items:\s*center;/);
  assert.match(css, /\.customer-auth-page--login \.customer-auth-stage\s*\{[\s\S]*?width:\s*min\(100%,\s*42rem\);[\s\S]*?display:\s*block;/);
  assert.match(css, /\.customer-auth-page--login \.customer-auth-stage__panel\s*\{[\s\S]*?backdrop-filter:\s*blur\(24px\) saturate\(140%\);/);
