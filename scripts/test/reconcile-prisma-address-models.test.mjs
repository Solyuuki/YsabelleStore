...
test("refuses partial models or unexpected anchors, without writing", () => {
  assert.throws(
    () => reconcileAddressModels(FIXTURE + "\nmodel CustomerSavedAddress { id String @id }\n"),
    /partial or custom/
  );
  assert.throws(
    () =>
      reconcileAddressModels(
        FIXTURE.replace("rememberedAuth      CustomerRememberedAuth[]", "unexpected String")
      ),
