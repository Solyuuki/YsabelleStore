...
assert.match(receivingSource, /@\/components\/ui\/sheet/);
assert.match(receivingSource, /@\/components\/ui\/table/);
assert.match(receivingSource, /role="tablist"/);
assert.doesNotMatch(receivingSource, /QueueSummaryCard/);
assert.doesNotMatch(receivingSource, /DialogContent/);
...
assert.match(routesSource, /path: "\/receiving"[\s\S]*?icon: ArrowDownToLine/);
