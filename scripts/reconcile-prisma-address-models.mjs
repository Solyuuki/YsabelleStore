...
function replaceModelOnce(text, modelName, oldLine, newLine) {
  ...
  if (indices.length !== 1)
    throw new Error(`Expected exactly one ${modelName}.${oldLine.trim()} anchor; no changes made.`);
  ...
  if (body.includes(newLine.trim()))
    throw new Error(`${modelName} has partially applied address relation; no changes made.`);
...
export function reconcileAddressModels(original) {
  ...
  if (hasSaved || hasSnapshot) {
    ...
    throw new Error(
      "Address model state is partial or custom; review manually rather than rewriting."
    );
  }
  let text = replaceModelOnce(
    original,
    "CustomerAccount",
    "rememberedAuth      CustomerRememberedAuth[]",
    "  savedAddress        CustomerSavedAddress?"
  );
  text = replaceModelOnce(
    text,
    "CustomerOrder",
