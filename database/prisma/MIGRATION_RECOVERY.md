...
The `20260818170000_add_product_reviews` SQL specifies a rating range CHECK, but the supplied schema export did not contain that constraint. Four data-integrity counts from the owner: 50 products with barcodes, zero mismatched barcode identity rows, zero products without inventory, and zero out-of-range review ratings. These *limited counts* do not prove all historical DML ran, nor do they establish constraint enforcement. The four-auth migration doctor passed but covers only those four contracts.
...
6. Once schema, fresh replay, data invariants, restore test, and environment-specific history/checksum plan pass independent review, request separate approval for live migration metadata / DDL operations. *Only then* begin PayMongo test-only, card-only Hosted Checkout changes.
