...
export function CustomerLoginPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  function handleSocialStart(provider: CustomerSocialAuthProvider) {
    ...
    try {
      ...
      setBusySocialProvider(null);
      setServerError(error instanceof Error ? error.message : "Unable to start social sign-in. Please try again.");
    }
  }
    ...
    <CustomerAuthFrame mode="login" navigate={navigate}>
      <div className="customer-auth-card">
        <div className="customer-auth-card__intro">
          <span className="customer-auth-card__icon" aria-hidden="true"><KeyRound size={22} /></span>
        ...
        {quickSignPanel === null ? (
          <>
            <form aria-busy={submitting} className="customer-auth-form" onSubmit={handleSubmit} noValidate>
              ...
              <label className="customer-auth-field" htmlFor="customer-login-identifier">
                ...
                <input aria-describedby={fieldErrors.identifier ? "customer-login-identifier-error" : undefined} aria-invalid={Boolean(fieldErrors.identifier)} autoComplete="username" id="customer-login-identifier" onChange={(event) => setIdentifier(event.target.value)} placeholder="Username, email, or 09XXXXXXXXX" type="text" value={identifier} />
                {fieldErrors.identifier ? <small id="customer-login-identifier-error">{fieldErrors.identifier}</small> : null}
              </label>

              <label className="customer-auth-field" htmlFor="customer-login-password">
                <span>Password</span>
                <span className="customer-auth-password">
                  <input aria-describedby={fieldErrors.password ? "customer-login-password-error" : undefined} aria-invalid={Boolean(fieldErrors.password)} autoComplete="current-password" id="customer-login-password" onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" type={showPassword ? "text" : "password"} value={password} />
                ...
                {fieldErrors.password ? <small id="customer-login-password-error">{fieldErrors.password}</small> : null}
              </label>

              <div className="customer-auth-forgot-row">
                <CustomerLink href="/account-recovery" navigate={navigate}>Forgot password?</CustomerLink>
              </div>
          ...
          <CustomerEmailAuthPanel onCancel={() => setQuickSignPanel(null)} onVerified={handleOtpVerified} />
        ) : (
          <CustomerMobileAuthPanel onCancel={() => setQuickSignPanel(null)} onVerified={handleOtpVerified} />
        )}

        <p className="customer-auth-switch">
          New to Ysabelle Store?{" "}
          <CustomerLink href="/register" navigate={navigate}>Create Account</CustomerLink>
        </p>
      </div>
    </CustomerAuthFrame>
  );
