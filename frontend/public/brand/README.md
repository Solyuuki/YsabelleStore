...
For Windows packaging, `electron-builder` receives the approved canonical PNG directly and performs the native Windows icon conversion itself. Electron runtime also loads that same approved PNG for the BrowserWindow and AppUserModel window details. This avoids relying on a hand-built ICO container while keeping the exact approved artwork as the single source.
