...
**Goal:** Build a deterministic, read-only reconciliation pipeline for the 472 SARIMA source identities and the 430 Google Drive image assets, then use the reviewed output to drive safe catalog cleanup and missing-image sourcing.
...
- [ ] **Step 4: Run targeted tests** and verify GREEN.
- [ ] **Step 5: Export the connected Drive inventory** and assert the manifest contains exactly 430 raw image files.
...
- [ ] **Step 6: Validate total accounting** so all 472 sources and all 430 raw images are represented exactly once in the audit model, except assets intentionally referenced in an explicit duplicate group.
...
- Spec coverage: branch/base, historical preservation, independent readiness states, Drive count, conservative matching, external sourcing, and no mutation-before-review are covered.
