...
test("routes tasks and paths to the smallest useful subsystem set", () => {
  assert.deepEqual(mapPathToSubsystems("backend/src/services/inventoryService.ts", config), [
    "inventory"
  ]);
...
test("returns compact task context with one relevant flow and at most eight files", async () => {
  ...
  assert.deepEqual(
    context.relatedFlows.map((flow) => flow.name),
    ["pos-sale-to-stock"]
  );
...
test("persists context, detects a changed file, and incrementally refreshes it", async () => {
  ...
  let status = await getContextStatus({
    rootDir: root,
    config: built.config,
    index: built.index,
    state: built.state
  ...
  const called = await handleMcpRequest(
    {
      ...
      id: 2,
      method: "tools/call",
      params: { name: "find_relevant_context", arguments: { task: "Fix inventory stock" } }
    },
    runtime
  );
  assert.deepEqual(JSON.parse(called.result.content[0].text).subsystems, ["inventory"]);
  const report = await runtime.reportMismatch({
    summary: "Path changed",
    affectedSubsystems: ["inventory"]
  });
  assert.equal(report.recorded, true);
  assert.match(
    await readFile(path.join(root, ".ysabelle-context/mismatches.jsonl"), "utf8"),
    /Path changed/
  );
});
...
test("STDIO MCP server supports modern discovery and legacy initialize", async () => {
  ...
  input.write(
    `${JSON.stringify({ jsonrpc: "2.0", id: 1, method: "server/discover", params: { _meta: { "io.modelcontextprotocol/protocolVersion": "2026-07-28" } } })}\n`
  );
  const discovery = JSON.parse(await waitForLine(output));
  assert.ok(discovery.result.supportedVersions.includes("2026-07-28"));
  input.write(
    `${JSON.stringify({ jsonrpc: "2.0", id: 2, method: "initialize", params: { protocolVersion: "2025-06-18", capabilities: {}, clientInfo: { name: "test", version: "1" } } })}\n`
  );
