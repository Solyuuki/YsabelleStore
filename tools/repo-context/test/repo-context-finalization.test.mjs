...
async function fixtureRepo() {
  ...
  await writeFile(path.join(root, 'backend/src/services/stockDomainService.ts'), 'export const mutate = 1;\n');
  await writeFile(path.join(root, 'backend/src/services/posService.ts'), 'export const sale = 1;\n');
  await writeFile(path.join(root, 'backend/src/modules/forecasting/forecast.service.ts'), 'export const forecast = 1;\n');
...
test('runtime ensureFresh clears stale mapped context before task retrieval', async () => {
  ...
  await writeFile(path.join(root, 'backend/src/services/inventoryService.ts'), 'export const stock = 3;\n');
...
test('MCP task lookup refreshes stale context automatically', async () => {
  const { root } = await fixtureRepo();
  const runtime = await createRepositoryContextRuntime({ rootDir: root });
  await writeFile(path.join(root, 'backend/src/services/inventoryService.ts'), 'export const stock = 4;\n');
  assert.equal((await runtime.getStatus()).stale, true);
...
test('CLI query refreshes stale context automatically while status stays diagnostic', async () => {
  ...
  await writeFile(path.join(root, 'backend/src/services/inventoryService.ts'), 'export const stock = 5;\n');
...
test('canonical guidance no longer routes routine work through retired sources', async () => {
  const config = JSON.parse(await readFile(path.join(projectRoot, 'config/repository-context.json'), 'utf8'));
  assert.equal(config.authoritativeSources.projectScope, 'docs/PROJECT-SCOPE.md');
  assert.equal(config.authoritativeSources.repositoryLayout, 'docs/architecture/03-folder-architecture.md');
  assert.equal(config.authoritativeSources.moduleOwnership, 'docs/architecture/08-module-ownership.md');
