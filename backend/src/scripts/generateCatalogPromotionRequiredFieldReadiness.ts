...
function toMarkdown(readiness: CatalogPromotionRequiredFieldReadiness) {
  ...
  return [
    ...
    `- Total units resolved: ${summary.unitResolved}`,
    `- Units still requiring review: ${summary.unitNeedsReview}`,
    `- Ready to create: ${summary.readyToCreate}`,
    "",
    "## Price Rule",
    "",
    "The workbook selling price is retained only as historical price evidence with meaning `LAST_RECORDED_HISTORICAL_PRICE_2025`. It is not a verified current selling price and does not unlock Product activation.",
    "",
    "## Unit Rule",
    "",
    "Unit evidence is resolved conservatively from explicit packaging language first, reviewed product-specific overrides where needed, and the reviewed catalog taxonomy for known discrete retail items. Pouch is represented as PACK because the current ProductUnit enum has no POUCH value. Unknown taxonomy remains `REVIEW_REQUIRED`; size or weight text alone is not treated as proof of the selling unit.",
    "",
    "## Rows",
    "",
