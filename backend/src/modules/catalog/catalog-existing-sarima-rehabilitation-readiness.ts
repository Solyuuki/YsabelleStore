...
export function buildExistingSarimaRehabilitationReadiness(input: {
  ...
  if (idSet.size !== ids.length) {
    fail(
      ...
      "identity allowlist contains duplicate product ids"
    );
  }
  ...
  return {
    summary: {
      ...
      barcodePresentUnverified: rows.filter((row) => row.barcodeReadiness === "PRESENT_UNVERIFIED")
        .length,
      ...
      catalogImageCandidateAssets: rows.reduce(
        (total, row) => total + row.catalogImageAssetFileIds.length,
        0
      )
    },
    rows
