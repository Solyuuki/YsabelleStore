...
test("customer recovery OTP material is six digits, HMAC protected, and valid for ten minutes", () => {
  ...
  assert.match(material.otpSalt, /^[a-f0-9]{32}$/);
  ...
  assert.equal(
    customerRecoveryOtpMatches(secret, material.otpSalt, material.verificationCode, material.otpHash),
    true
  );

  const wrongCode = material.verificationCode === "000000" ? "000001" : "000000";
  assert.equal(customerRecoveryOtpMatches(secret, material.otpSalt, wrongCode, material.otpHash), false);
});
