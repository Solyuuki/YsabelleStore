  ...
  return {
    async cleanup() {
      const [
        ...
        findNewIds(prisma.customerAccount, baseline.customerAccounts),
      ...
      await prisma.$transaction(async (transaction) => {
        ...
        if (newCustomerSessions.length > 0) {
          await transaction.customerSession.deleteMany({
            where: { id: { in: newCustomerSessions } }
          });
        }

        if (newCustomerAccounts.length > 0) {
          await transaction.customerAccount.deleteMany({
            where: { id: { in: newCustomerAccounts } }
          });
        }
