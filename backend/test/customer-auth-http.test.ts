  ...
  try {
    await withServer(async (baseUrl) => {
      ...
      const missingAccount = await fetch(`${baseUrl}/api/customer-auth/login`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ email: `missing-${suffix}@example.com`, password: "wrong-password" })
      });
  ...
  try {
    ...
    await withServer(async (baseUrl) => {
      const register = await fetch(`${baseUrl}/api/customer-auth/register`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: "Boundary Customer", email: customerEmail, password: PASSWORD })
      });
      assert.equal(register.status, 201);
